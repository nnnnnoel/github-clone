...
import { ForkHistory, Repository } from '../../../../../@types';
...
const Fork: React.FC<Props> = ({ item }) => {
  ...
  }, [item.repo.url, getRepositoryInfo]);
      ...
      <ContentContainer>
        <div>
          <HeaderContainer>
            <div>
              ...
              {' forked '}
              <HeaderAnchor>{' ' + item.payload?.forkee?.full_name + ' '}</HeaderAnchor>
              {' from '}
        ...
        <Box>
          ...
          <div className='dashboard-break-word text-gray mt-1 repo-description'>
            <p>
              {(item.payload?.forkee?.description?.length || 0) > 147
                ? item.payload?.forkee?.description?.substr(0, 147) + '...'
                : item.payload?.forkee?.description}
            </p>
          </div>
...
export default React.memo(observer(Fork));
